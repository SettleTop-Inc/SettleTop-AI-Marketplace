-- registry_search: p_source accepts marketplace IDS as well as NAMES.
--
-- The only change from 20260820100100_registry_search_asset_keyed.sql is how
-- p_source is handled. Everything else in this file is that definition carried
-- over byte for byte: the twelve parameters and their order, the ranking, the
-- self-excluding facet counts, the six other facets, the paging and the
-- deferred row serialisation. The return shape is unchanged. lib/registry.ts
-- and lib/registry-search.parity.test.ts both read this shape and neither moves.
--
--
-- THE DEFECT this fixes (issue #47).
--
-- The previous definition compared p_source against f_sources, which is the set
-- of marketplace NAMES the asset is listed on. So a p_source element that was a
-- marketplace NAME matched, and a p_source element that was a marketplace ID did
-- not. On production that meant:
--
--   p_source {'Microsoft Marketplace'} -> 6855   (a name: matched)
--   p_source {'microsoft'}             -> 0       (an id: matched nothing)
--
-- and worse than the empty result, the unrecognised 'microsoft' was still
-- unnested verbatim into the `selected` CTE and seeded into the source facet as
-- a real ticked bucket at count 0: a phantom value the rail could show and the
-- filter could never satisfy.
--
--
-- THE FIX. p_source now accepts BOTH ids and names. Every element is resolved to
-- the canonical marketplace NAME before it is compared to f_sources, and the
-- facet is seeded with the RESOLVED name, deduplicated. So:
--
--   'microsoft'            resolves to 'Microsoft Marketplace' and matches
--   'Microsoft Marketplace' resolves to itself and matches (names unchanged)
--   both together          are ONE selection: 6855, not 13710 and not 0
--   an unknown value       resolves to nothing: total 0, and seeds NO facet
--                          value, so there is no phantom bucket
--
-- The trap, named in the issue and avoided here: the seed and the facet's
-- `selected` flag both read the RESOLVED name, never the raw input. Seeding the
-- raw input would just move the phantom bucket from the name's spelling to the
-- id's spelling ('microsoft' at count 0) instead of removing it. src_resolved
-- is the single source of the resolved names, and the filter, the seed and the
-- selected flag all read it.
--
--
-- THE RESOLUTION MAP is marketplace.id -> marketplace.name AND
-- marketplace.name -> marketplace.name, unioned: an input matching either an id
-- or a name resolves to that marketplace's canonical name. This is well defined
-- only if no marketplace id equals a DIFFERENT marketplace's name; otherwise a
-- single input would resolve to two marketplaces. Today's three marketplaces
-- are aws/'AWS Marketplace', drai/'DRAI Agentic-AI Marketplace' and
-- microsoft/'Microsoft Marketplace', and no id equals any name. The DO block
-- below asserts that invariant at migration time, so if a future marketplace
-- ever breaks it the migration fails loudly here rather than the function
-- silently double-counting one input.
--
-- The lookup is against the same two-to-three-row marketplace table
-- v_registry_card already inner-joins, so it adds no dependency anon did not
-- already have: if anon could not read marketplace the card would be empty and
-- total would be 0 before this function ever ran.
...
begin
  select count(*) into n
    from marketplace a
    join marketplace b on a.id = b.name
   where a.id <> b.id;
  if n > 0 then
    raise exception
      'registry_search source resolution is ambiguous: % marketplace id(s) equal a different marketplace''s name. Define the resolution order before shipping this migration.', n;
  end if;
end $$;
...
-- 42501 and the registry page has no results at all. registry_search is
-- security invoker (declared above), so this reissue also confirms the drop and
-- recreate did not silently change the executable-by set.
