create or replace function registry_safe_date(p text)
...
-- ingest_capture is the single write path into the registry.
--
-- It takes one capture payload and does everything: upserts the asset, stores
-- the observation whole, explodes the extract into queryable rows, computes
-- every derived value, and records what moved since the previous capture.
--
-- The honesty gate lives HERE, not in the caller. Every stated build value is
-- re-checked against this capture's own text before it is marked verified, so
-- a careless or over-eager writer cannot introduce a claim the source does not
-- make. Public views read verified rows only.
--
-- Idempotent on drive_file_id: replaying the same Drive file is a no-op.
...
begin
  ...
  -- replaying a Drive file we already hold is a no-op, not an error
  ...
  on conflict (marketplace_id, source_product_id)
    do update set updated_at = now()
  returning id into v_asset;

  select id into v_prev from capture
   where asset_id = v_asset order by captured_at desc limit 1;

  v_hash := encode(digest(ex::text, 'sha256'), 'hex');

  insert into capture (asset_id, captured_at, template_version, capture_complete,
                       missing, source_view_url, drive_file_id, drive_file_name,
                       raw, content_hash, ingest_source)
  values (v_asset, v_captured,
  ...
  -- text this capture actually contains, split by who is asserting it
  ...
  for k in select jsonb_object_keys(coalesce(ex -> 'stated', '{}'::jsonb)) loop
    if kindmap ? k then
      for val in select jsonb_array_elements_text((ex -> 'stated') -> k) loop
        if val is null or trim(val) = '' then continue; end if;
  ...
  -- a certification page naming Graph scopes IS evidence of Graph use
  ...
  -- ------- derived, computed only from what was just stored ---------------
  if nullif(ex ->> 'publisher','') is not null then layers := layers || 'vendor identity'; end if;
  if exists (select 1 from capture_evidence where capture_id = v_capture and kind = 'model' and verified)
    then layers := layers || 'model'; end if;
  if exists (select 1 from capture_evidence where capture_id = v_capture and kind = 'framework' and verified)
    then layers := layers || 'framework'; end if;
  if exists (select 1 from capture_evidence where capture_id = v_capture and kind = 'tool_mcp' and verified)
     or array_length(perms,1) > 0
    then layers := layers || 'tools and MCP'; end if;
  if exists (select 1 from capture_evidence where capture_id = v_capture and kind = 'data_source' and verified)
    then layers := layers || 'data sources'; end if;
  if exists (select 1 from capture_evidence where capture_id = v_capture and kind = 'integration' and verified)
     or array_length(works,1) > 0
    then layers := layers || 'integrations'; end if;
  if nullif(cert_d ->> 'hosting','') is not null then layers := layers || 'hosting'; end if;
  if nullif(cert_d ->> 'data_location','') is not null then layers := layers || 'data residency'; end if;
  if v_cert in ('microsoft_365_certified','publisher_attestation') then layers := layers || 'permission scope'; end if;
  if nullif(ex ->> 'pricing','') is not null or plan_count > 0 then layers := layers || 'pricing'; end if;
  if nullif(ex ->> 'acquire_using','') is not null then layers := layers || 'access model'; end if;
  if nullif(ex ->> 'support','') is not null then layers := layers || 'support channel'; end if;

  v_reach := round(100.0 * coalesce(array_length(layers,1),0) / array_length(registry_layers(),1));
  v_risk  := registry_risk(v_cert, coalesce(array_length(layers,1),0));
  v_prov  := registry_provenance(v_cert);
  v_price := registry_price(ex ->> 'pricing', plan_count);

  select coalesce(fo.function_category,
                  registry_function_category(ex ->> 'name', ex ->> 'tagline', cats))
    into v_fn
    from (select 1) _
    left join function_override fo
      on fo.marketplace_id = v_mkt and fo.source_product_id = v_pid;
  ...
  -- ------- what moved since the previous observation ----------------------
  if v_prev is not null then
    select jsonb_build_object(
      ...
      'evidence', (select coalesce(jsonb_object_agg(kind, vals),'{}') from (
                     select kind::text as kind, jsonb_agg(value order by value) as vals
                       from capture_evidence where capture_id = v_prev and verified
                      group by kind) s)
    ...
    select jsonb_build_object(
      ...
      'evidence', (select coalesce(jsonb_object_agg(kind, vals),'{}') from (
                     select kind::text as kind, jsonb_agg(value order by value) as vals
                       from capture_evidence where capture_id = v_capture and verified
                      group by kind) s)
    ...
    for fld in select jsonb_object_keys(new_sig) loop
      if (old_sig -> fld) is distinct from (new_sig -> fld) then
        insert into asset_change (asset_id, from_capture_id, to_capture_id,
                                  observed_at, field, old_value, new_value)
        values (v_asset, v_prev, v_capture, v_captured, fld,
                old_sig -> fld, new_sig -> fld);
        changes := changes + 1;
      end if;
    end loop;
  end if;
  ...
  update asset
     set current_capture_id = v_capture,
